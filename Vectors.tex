...
\begin{enumerate}
	...
	\item \begin{enumerate}[label=(\alph*)]
     ...
     \item Students of a school are standing in rows and columns in their school playground to celebrate their annual sports day. A, B, C and D are the positions of four students as shown in the figure. \\
	     ...
	     \begin{figure}[ht]
		     \centering
		     \includegraphics[width=6cm,height=6cm]{fwc3.png}
	     \end{figure}
	     
	     \vspace{5mm}
Based on the above, answer the following question :
\begin{enumerate}[label=(\roman*)]
	\item The figure formed by the points A, B, C and D is a
		\begin{enumerate}[label=(\Alph*)]
			\item sqaure
			\item parallelogram
			\item rhombus
...
\end{enumerate}
\item The coordinate of the three consecutive vertices of a parallelogram $\vec{ABCD}$ are $\vec{A}(1, 3)$, $\vec{B}(-1, 2)$, and $\vec{c}(2, 5)$. Find the cordinates of the fourth vertex D.

\item \begin{enumerate}[label=(\alph*)]
		\item If $\vec{p}(2, 2)$, $\vec{Q}(-4, -4)$ and $\vec{R}(5, -8)$ are the vertices of a $\triangle$ $\vec{PQR}$, then find the length of te median through R.
...
\item
	\begin{enumerate}[label=(\alph*)]
		...
		\item The points $\vec{A}(0, 3)$, $\vec{B}(-2, a)$ and $\vec{C}(-1, 4)$ are the vertices of a rigth triangle, right-angled at A. Find the value of a. 
	\end{enumerate}
\end{enumerate}
\end{document}
